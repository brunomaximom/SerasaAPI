...
import time, math, pandas, redis


def process_table(conn, offset, current_url):
    """
    Processo para cada página da tabela em paralelo
    """
    driver = Chrome()
    driver.get(current_url+"?offset="+str(offset)+"&count=250")     # recarregar a página com os parâmetros adequados
...
def crawler(region):
    ...
    while offset <= int(nstocks):
        p = Process(target=process_table, args=(conn,offset,current_url))
        p.start()
        p.join(3)
        offset += 250
